...
%         [GE.dfgrf]=get_gaitEvents_GRF(forces_df.FP1For,forces_df.FP2For);
        ...
        markers_df_c.labels(1,:) = {'LASIS', 'RASIS', 'LPSIS', 'RPSIS', 'LKNE', 'LTHI', 'LANK', 'LTIB', 'LTOE', 'LHEE', 'RKNE', 'RTHI', 'RANK', 'RTIB', 'RTOE', 'RHEE'};
        llmarkers= markers_df;
        [RHS,LTO,LHS,RTO,GE,GEInMiddleDeleted] = GaitEvents_mocap_v7(Time,llmarkers,markers4GE, HSrefinePre, HSrefinePost, TOminpeakdistance, TOminpeakheight,BW); 
...
%         
        ...
        for sn = 1:length(GEtreadmill), i=2:length(GEtreadmill);
...
%         plot(GEgood(:,1), zeros(size(GEgood(:,1))), 'rx', GEgood(:,2), zeros(size(GEgood(:,1))), 'bo', GEgood(:,3), zeros(size(GEgood(:,1))), 'bx', GEgood(:,4), zeros(size(GEgood(:,1))), 'ro', GEgood(:,5), zeros(size(GEgood(:,1))), 'rx')
        plot(GEgood(:,1), zeros(size(GEgood(:,1))), 'rx', GEgood(:,2), zeros(size(GEgood(:,1))), 'bo', GEgood(:,3), zeros(size(GEgood(:,1))), 'bx', GEgood(:,4), zeros(size(GEgood(:,1))), 'ro')
        ylimits = ylim(gca);
...
%         sw(:,2) = stepwidth(markers_df_c.LHEE(:,1), markers_df_c.RHEE(:,1), GEgood(:,3));
        ...
        sl(:,1) = steplength(markers_df_c.RHEE(:,2), markers_df_c.LHEE(:,2), GEgood(41:end,1));
        sw(:,1) = stepwidth(markers_df_c.RHEE(:,1), markers_df_c.LHEE(:,1), GEgood(41:end,1));
        
        sl(:,2) = steplength(markers_df_c.LHEE(:,2), markers_df_c.RHEE(:,2), GEgood(41:end,3));
        sw(:,2) = stepwidth(markers_df_c.LHEE(:,1), markers_df_c.RHEE(:,1), GEgood(41:end,3));
        ...
        sl = tempsl;
        sw = tempsw;
        sp = avespeed(goodsl(30:end))';
%         sp = avespeed(39:end)';
        for sn = 1:length(GEgood)
            st(:,1) = Time_df(GEgood(sn,5))-Time_df(GEgood(sn,1));
        end
