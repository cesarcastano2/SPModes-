...
% annotation('textarrow',[0.20 0.21],[0.3 0.4])
...
if c == 2 
stride_speed_all_barplot_incline(s) = mean(stride_speed_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400)));
end 
if c == 3 
stride_speed_all_barplot_decline(s) = mean(stride_speed_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400)));
end 
%% Stride speeds, lengths, frequency for all subjects 
     ...
     if c == 2
     speed_strides.incline(s,:) = stride_speed_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400));
     end
     if c == 3
     speed_strides.decline(s,:) = stride_speed_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400));
     end
     ...
     if c == 2
     length_strides.incline(s,:) = stridelength_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400));
     end
     if c == 3
     length_strides.decline(s,:) = stridelength_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400));
     end
     ...
     if c == 2
     time_strides.incline(s,:) = stridelength_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400));
     end
     if c == 3
     time_strides.decline(s,:) = stridelength_all(STV_OP1.level.steadystatevalue(s):(STV_OP1.level.steadystatevalue(s)+400));
     end
%% BEST FIT MODEL 
% v = 0.5:0.1:1.8;
%     figure(100)
...
%     plot(v,fh(v,P),'Color',color(c,:));
    ...
    p.decline_width(s,:) = W;
     ...
     if c == 2
     variation_strides_width.incline.slminusfit(s,:) = var(width_strides.incline(s,:) - fh_width(speed_strides.incline(s,:),W));
     variation_strides_width.incline.speedtrend(s,:) = var(fh_width(speed_strides.incline(s,:),W));
     variation_strides_width.incline.totalvar(s,:) = var(width_strides.incline(s,:));
     end
     if c == 3
     variation_strides_width.decline.slminusfit(s,:) =  var(width_strides.decline(s,:) - fh_width(speed_strides.decline(s,:),W));
     variation_strides_width.decline.speedtrend(s,:) = var(fh_width(speed_strides.decline(s,:),W));
     variation_strides_width.decline.totalvar(s,:) = var(width_strides.decline(s,:));
     end
...
%% STRIDE LENGTH VARIANCE PLOTS

% figure 
% plot(variation_strides.level.totalvar) 
% hold on 
% plot(variation_strides.level.speedtrend)
% plot(variation_strides.level.slminusfit)
% 
% figure 
% plot(variation_strides.incline.totalvar) 
% hold on 
% plot(variation_strides.incline.speedtrend)
% plot(variation_strides.incline.slminusfit)
% 
% figure 
% plot(variation_strides.decline.totalvar) 
% hold on 
% plot(variation_strides.decline.speedtrend)
% plot(variation_strides.decline.slminusfit)
% 
...
% err = [0.003364497235436,7.753067646392686e-04,0.003469192919694;0.004441625434736,2.884199498261776e-04,0.004488014335807;0.012620078918318,9.188888273492992e-04,0.010671619462801];
...
hold off
% errorbar([1:3;4:6;7:9],[mean(variation_strides.decline.totalvar),mean(variation_strides.decline.slminusfit),mean(variation_strides.decline.speedtrend);mean(variation_strides.level.totalvar),mean(variation_strides.level.slminusfit),mean(variation_strides.level.speedtrend);mean(variation_strides.incline.totalvar),mean(variation_strides.incline.slminusfit),mean(variation_strides.incline.speedtrend)],[std(variation_strides.decline.totalvar),std(variation_strides.decline.slminusfit),std(variation_strides.decline.speedtrend);std(variation_strides.level.totalvar),std(variation_strides.level.slminusfit),std(variation_strides.level.speedtrend);std(variation_strides.incline.totalvar),std(variation_strides.incline.slminusfit),std(variation_strides.incline.speedtrend)],'.','Color','k')
% errorbar([0.007325047418686,0.001396645976994,0.005861722017405;0.005845360152157,5.275793822049252e-04,0.005320256722543;0.010533336259934,0.001651347330162,0.007143693371322], [0.003364497235436,7.753067646392686e-04,0.003469192919694;0.004441625434736,2.884199498261776e-04,0.004488014335807;0.012620078918318,9.188888273492992e-04,0.010671619462801])
ylim([0 0.025])
